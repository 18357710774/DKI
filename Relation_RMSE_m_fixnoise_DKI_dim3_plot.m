...
load([loadpath '\syn_results\Relation_RMSE_m_fixnoise_DKI_dim3.mat'], ...
    'delta_seq', 'm_seq', 'RMSE_DKI_mean', 'TC_train_DKI_mean', ...
    'TC_predict_DKI_mean', 'RMSE_DKI', 'TC_predict_DKI', 'TC_train_DKI');
...
for k = 1:length(noise_ind_sel)
    semilogy(m_seq, RMSE_DKI_mean(noise_ind_sel(k),:), 'LineWidth', Linewidth_val(k), 'Color', mycolor(k,:));
    hold on;
end
...
legend(str_legend, 'Location','southeast','NumColumns',1,'FontSize',12);
